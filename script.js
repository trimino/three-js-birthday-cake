...
function getFlameMaterial(isFrontSide) {
	...
	return new THREE.ShaderMaterial({
      ...
      void main() {
        ...
        pos.y *= 1. +
          (cos((posXZlen + 0.25)*3.14159)*0.25 +
...
function render() {
	...
	candleMesh.children.forEach(child => {
		if (child instanceof THREE.PointLight && child.distance === 10) {
			child.position.x = Math.sin(time * Math.PI) * 0.25;
			child.position.z = Math.cos(time * Math.PI * 0.75) * 0.25;
			child.intensity = 2 + Math.sin(time * Math.PI * 2) * Math.cos(time * Math.PI * 1.5) * 0.25;
		}
	});
...
function createCake() {
	...
	midMesh.position.y = baseHeight / 2 + middleHeight / 2; // 1 + 0.625 = 1.625
	...
	const topMesh = new THREE.Mesh(topGeo, topMat);
	topMesh.position.y = baseHeight / 2 + middleHeight + topHeight / 2; // 1 + 1.25 + 0.5 = 2.75
...
candleMesh.position.y = baseHeight / 2 + middleHeight + topHeight; // 1 + 1.25 + 1 = 3.25
...
// ─────────────────────────────────────────────────────────────────────────────
// 7) ADD STRAWBERRIES ON EACH LAYER
// ─────────────────────────────────────────────────────────────────────────────
function createStrawberry() {
	...
	strawberry.add(bodyMesh);

	// Green leafy top
	const leafGeo = new THREE.ConeGeometry(0.12, 0.2, 6);
	const leafMat = new THREE.MeshStandardMaterial({ color: 0x388e3c });
	const leafMesh = new THREE.Mesh(leafGeo, leafMat);
	leafMesh.position.y = 0.15;
	leafMesh.rotation.x = Math.PI;
	strawberry.add(leafMesh);

	return strawberry;
}

// 7.1) Strawberries on top of base layer (ring around baseRadius - 0.3)
const baseStrawberryGroup = new THREE.Group();
const baseStrawberryCount = 8;
const baseRingRadius = baseRadius - 0.3; // ~2.2
for (let i = 0; i < baseStrawberryCount; i++) {
	const angle = (i / baseStrawberryCount) * Math.PI * 2;
	const berry = createStrawberry();
	berry.position.x = Math.cos(angle) * baseRingRadius;
	berry.position.z = Math.sin(angle) * baseRingRadius;
	// **Place on top of base**: base top = y = +1
	berry.position.y = 1 + 0.2; // 1.2
	baseStrawberryGroup.add(berry);
}
cake.add(baseStrawberryGroup);

// 7.2) Strawberries on top of middle layer (ring around middleRadius - 0.3)
const middleStrawberryGroup = new THREE.Group();
const middleStrawberryCount = 6;
const middleRingRadius = middleRadius - 0.3; // ~1.7
for (let i = 0; i < middleStrawberryCount; i++) {
	const angle = (i / middleStrawberryCount) * Math.PI * 2;
	const berry = createStrawberry();
	berry.position.x = Math.cos(angle) * middleRingRadius;
	berry.position.z = Math.sin(angle) * middleRingRadius;
	// **Place on top of middle**: middle top = baseHeight/2 + middleHeight/2 + middleHeight/2 = 2.25
	berry.position.y = 2.25 + 0.2; // 2.45
	middleStrawberryGroup.add(berry);
}
cake.add(middleStrawberryGroup);

// 7.3) Strawberries on top of top layer (ring around topRadius - 0.3)
const topStrawberryGroup = new THREE.Group();
const topStrawberryCount = 4;
const topRingRadius = topRadius - 0.3; // ~1.2
for (let i = 0; i < topStrawberryCount; i++) {
	const angle = (i / topStrawberryCount) * Math.PI * 2;
	const berry = createStrawberry();
	berry.position.x = Math.cos(angle) * topRingRadius;
	berry.position.z = Math.sin(angle) * topRingRadius;
	// **Place on top of top layer**: top top = baseHeight/2 + middleHeight + topHeight/2 + topHeight/2 = 3.25
	berry.position.y = 3.25 + 0.2; // 3.45
	topStrawberryGroup.add(berry);
}
cake.add(topStrawberryGroup);
...
// ─────────────────────────────────────────────────────────────────────────────
// 8) LOAD FONT AND CREATE “Happy Birthday Kelsei” TEXT (AFTER CAKE EXISTS)
// ─────────────────────────────────────────────────────────────────────────────
...
fontLoader.load(
	...
	(font) => {
		...
		// Position it above the top strawberries / candles: y = 4.80
...
// ─────────────────────────────────────────────────────────────────────────────
// 9) “HOLD TO BLOW OUT” LOGIC & CONGRATULATION OVERLAY
// ─────────────────────────────────────────────────────────────────────────────
let holdTimeout;
